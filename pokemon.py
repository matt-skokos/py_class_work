...
class Pokemon:
    ...
    def __str__(self):
        return f"{self.name}'s trainer is {self.trainer} and their " \
               f"basic attack is '{self.basic_attack}'"
...
class PsychicType(Pokemon):
    ...
    def __str__(self):
        return f"{self.name}'s trainer is {self.trainer} and " \
               f"their basic attack is '{self.basic_attack}'"
    ...
    def attack(self, other):
        if not self.paralyzed:
            ...
            if isinstance(other, (FightingType, DragonType, DarkType)):
                ...
                other.receive_damage(self.damage)
            if random() < self.prob:  # chance of inflicting Confusion
                other.confused = True
                print(f"{other.name} is confused!")
        else:
            print(f"{self.name} could not attack. {self.name} "
                  f"is paralyzed!")
...
def main():
    pass
    '''     --- TEST CASES FOR Pokemon Objects ---       '''
    #
